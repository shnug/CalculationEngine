	...
	public class CalculationEngine : IDisposable, ICalculationEngine
	{
		...
		private double Calculate(PhraseType pt)
		{
			...
			switch (pt)
			{
				...
				//log series
				case PhraseType.ln:
					return Math.Log(a, _context.Variables.Get("e"));
				case PhraseType.log:
		...
		public void Run()
		{
			...
			while (i < _context.Phrases.Length)
			{
				...
				if (temp_pt == PhraseType.number)
					...
					_opnd.Push(_context.Variables.EXP);
				else if (temp_pt == PhraseType.pi)
					_opnd.Push(_context.Variables.PI);
				else if (temp_pt == PhraseType.ans)
					_opnd.Push(_context.Variables.ANS);
				else if (temp_pt == PhraseType.variable)
					_opnd.Push(_context.Variables.Get(_context.Phrases.PhraseResult[i] as string));
				else //is operator
				{
					//calculation stop
					...
					PriorityCmpType temp_pct = Operator.OperatorCmp2((PhraseType) _optr.Peek(), temp_pt);
					if (temp_pct == PriorityCmpType.Higher)
					{
						...
						} while ((PriorityCmpType) Operator.OperatorCmp2((PhraseType) _optr.Peek(), temp_pt) ==
						         PriorityCmpType.Higher);

						//当相邻PhraseType优先级相等时
						if ((PriorityCmpType) Operator.OperatorCmp2((PhraseType) _optr.Peek(), temp_pt) ==
						    PriorityCmpType.Equal)
						{
			...
			//save result to ans
			_context.Variables.ANS = (double) _opnd.Peek();
		}

		public double Calculate()
		{
			this.Run();
			return _context.Variables.ANS;
		}
	}
}
